 ...
 * thinking / tool parts and an `isStreaming` ref, and pushes chunks
 * into the tree on its own clock. Consumers wire `messages.value` into
 * the template and `isStreaming` into `useChatScroll({ streaming })`.
 * They never see the underlying chunk shape.
 */
