...
export function useFakeChat<TMessage = DefaultChatMessage, TChunk = string>(
  ...
  const messages = ref<TMessage[]>(opts.initial ?? []) as Ref<TMessage[]>
  const status = ref<ChatStatus>('idle') as Ref<ChatStatus>
  const lastUser = ref<TMessage | null>(null) as Ref<TMessage | null>
  ...
  if (getCurrentInstance()) onBeforeUnmount(stopAutoTimer)
  ...
  function tick(): boolean {
    ...
    if (assistantId === null) {
      ...
      messages.value = messages.value.map((m) =>
        idOf(m) === id ? applyChunk(m, chunk) : m,
      )
    }
    chunkIdx += 1
